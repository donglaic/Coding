...
def write_to_file(content):
    with open('lianjiaResult_hymj.csv','a',encoding='utf-8') as f:
        fieldnames = ['title','dealDate','unitPrice','totalPrice']
        writer = csv.DictWriter(f,fieldnames=fieldnames)
        writer.writerow(content)

def main(page):        
    url = 'https://cd.lianjia.com/chengjiao/{page}c3011055413783/'.format(page=page)
    for item in parse_one_page(url):
        print(item)
        write_to_file(item)

if __name__ == '__main__':
    for page in ['','pg2','pg3','pg4','pg5']:
        main(page)
        time.sleep(1)
